...
function ExploreSection({ config }: { config: ExploreSectionConfig }) {
  return (
    <div>
...
export default async function ExplorePage() {
  ...
  const exploreSectionsWithEnrichedImages: ExploreSectionConfig[] = [
    { ...exploreSections[0], items: topActivities },
    { ...exploreSections[1], items: topBeaches },
    { ...exploreSections[2], items: topPlaces },
  ];
  ...
  return (
    <main id="main-content" className="min-h-screen">
      ...
      <section className="relative h-[44vh] min-h-[280px]">
        <Image
          src={getImageUrl(
            "/images/banners/le-morne-aerial-view-mauritius.jpg",
          )}
          alt="Explore Mauritius"
          fill
          priority
          fetchPriority="high"
          className="object-cover"
        ...
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <div className="text-center px-4 max-w-4xl mx-auto translate-y-[2rem]">
            ...
            <p className="text-white/90 text-base md:text-lg max-w-2xl mx-auto">
              Beaches, places to visit and activities — everything you need to
              discover the island
            </p>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-8">
              <span className="text-orange-500 font-medium tracking-wider text-sm uppercase">Discover</span>
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mt-2">
                Explore Top Picks Across Mauritius
              </h2>
              <p className="text-gray-600 mt-4 max-w-2xl mx-auto">
                From thrilling activities to pristine beaches and must-see landmarks — start exploring here
              </p>
            </div>

            <div className="grid md:grid-cols-3 gap-8">
              {exploreSectionsWithEnrichedImages.map((section) => (
                <ExploreSection key={section.title} config={section} />
              ))}
            </div>
          </div>
        </div>
      ...
      {/* <section className="pt-6 pb-4 bg-white">
        <div className="container mx-auto px-4 mb-5">
          ...
          <p className="text-gray-600 text-center max-w-2xl mx-auto">
            Secret beaches and secluded spots that only locals know about — away from the crowds
          </p>
        </div>
        <div className="w-full pt-4 pb-4 space-y-0">
          <div className="relative">
            <Marquee className="[--duration:70s] [--gap:0.375rem] sm:[--gap:0.75rem]">
              {[...topActivities, ...topActivities].map((item, index) => (
                <MarqueeCard key={`act-${item.slug}-${index}`} item={item} label="Activity" href={`/mauritius-activities/${item.slug}`} />
              ))}
            </Marquee>
          ...
          <div className="relative mt-1">
            <Marquee reverse className="[--duration:70s] [--gap:0.375rem] sm:[--gap:0.75rem]">
              {[...topBeaches, ...topBeaches].map((item, index) => (
                <MarqueeCard key={`beach-${item.slug}-${index}`} item={item} label="Beach" href={`/beaches-in-mauritius/${item.slug}`} />
              ))}
            </Marquee>
            <div className="pointer-events-none absolute inset-y-0 left-0 w-16 md:w-24 bg-gradient-to-r from-white" />
            <div className="pointer-events-none absolute inset-y-0 right-0 w-16 md:w-24 bg-gradient-to-l from-white" />
          ...
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 mt-3 max-w-xl mx-auto">
                Secret beaches that only locals know about — away from the
                crowds
              </p>
            </div>

            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 max-w-4xl mx-auto">
              {hiddenGems.map((beach) => (
                <SmallCard
                  key={beach.slug}
                  {...beach}
