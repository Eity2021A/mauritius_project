...
import { type ActivityCategory, type Region, REGION_BADGE_COLORS } from "@/types/content";
...
function ActivityCard({ activity, priority = false, slugsWithPages, categoryLabels, regionLabels }: { activity: Activity; priority?: boolean; slugsWithPages: Set<string>; categoryLabels: Record<ActivityCategory, string>; regionLabels: Record<Region, string> }) {
  ...
  const firstCategoryLabel = firstCategory ? (categoryLabels[firstCategory] ?? firstCategory) : null;
  const regionLabel = region ? (regionLabels[region] ?? region) : null;
...
export default function ActivitiesListClient(props: ActivitiesListClientProps) {
  const searchParams = useSearchParams();
  const categoryParam = searchParams.get("category");

  return (
    <ActivitiesListClientContent
...
function ActivitiesListClientContent({
  allActivities,
  activityCategories,
  activitySlugsWithPages: slugsWithPagesProp,
  categoryParam,
  ...
  const translatedActivityCategories = activityCategories.map((category) => ({
    ...category,
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <p className="text-gray-700 leading-relaxed text-center [&_strong]:text-orange-600 [&_strong]:font-semibold">
              {t("intro.beforeAdventure")} <strong>{t("intro.adventure")}</strong>{t("intro.afterAdventure")} <strong>{t("intro.cruises")}</strong> {t("intro.and")} <strong>{t("intro.boatTours")}</strong>{t("intro.afterCruises")} <strong>{t("intro.hiking")}</strong>{t("intro.afterHiking")} <strong>{t("intro.snorkeling")}</strong> {t("intro.and")} <strong>{t("intro.scuba")}</strong>{t("intro.afterScuba")}
            </p>
          </div>
        </div>
      </section>
              ...
              >
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {translatedActivityCategories.map((category) => {
