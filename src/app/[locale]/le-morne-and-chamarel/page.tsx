...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: t.metadata.title, description: t.metadata.description, alternates: { canonical: "/le-morne-and-chamarel" } };
}

export default async function LeMorneAndChamarelPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = getExploringGuide(locale, "morneChamarel");
  return (
