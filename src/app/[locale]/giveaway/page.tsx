...
import { getDetailPageTranslations } from "@/data/detail-page-translations";
import {
...
function getLocalizedCountryName(countryName: string, locale: string) {
  if (locale === "en") return countryName;
  try {
    const intlWithValues = Intl as typeof Intl & {
      supportedValuesOf?: (key: "region") => string[];
    };
    const regionCodes = intlWithValues.supportedValuesOf?.("region") ?? [];
    const englishNames = new Intl.DisplayNames(["en"], { type: "region" });
    const countryCode = regionCodes.find((code) => englishNames.of(code) === countryName);
    if (!countryCode) return countryName;
    return new Intl.DisplayNames([locale], { type: "region" }).of(countryCode) ?? countryName;
  } catch {
    return countryName;
  }
}
...
export default function GiveawayPage() {
  ...
  const labels = getDetailPageTranslations(locale).giveaway;
  ...
  const shareToInstagram = async () => {
    ...
    try {
      ...
      window.prompt(labels.copyPrompt, u);
    }
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center text-gray-900 mb-8">
            <p className="text-lg md:text-xl font-medium">
              {labels.drawOn} <strong className="text-orange-600">{campaign.draw_date_label}</strong> ({labels.mauritiusTime})
            </p>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 md:p-10">
              <h3 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2 text-center">{labels.enterTitle}</h3>
              ...
              {submitted ? (
                <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
                  ...
                  <h4 className="text-2xl font-bold text-green-700 mb-2">{labels.submittedTitle}</h4>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    >
                      ...
                      {COUNTRY_OPTIONS.map((c) => (
                        <option key={c} value={c}>
                          {getLocalizedCountryName(c, locale)}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="bg-slate-50 rounded-xl p-5 md:p-6 space-y-4 border border-slate-100">
                    <h4 className="font-semibold text-gray-900 flex flex-wrap items-center gap-2">
                      <span>{t("shareThisGiveaway")}</span>
                    </h4>
                    <p className="text-sm text-gray-600">
                      {labels.shareInstruction}
                    </p>
                  ...
                  <div className="bg-blue-50 rounded-lg p-6 space-y-4 border border-blue-100/80">
                    ...
                    <p className="text-sm text-gray-600">
                      {labels.participateBefore} <strong>mauritiusexplored.com</strong> {labels.participateAfter}
                    </p>
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group">
                      ...
                      <span className="text-sm text-gray-700 group-hover:text-gray-900">
                        {labels.facebookConfirmBefore} <strong>mauritiusexplored.com</strong> {labels.facebookConfirmAfter}{" "}
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group">
                      ...
                      />
                      <span className="text-sm text-gray-700 group-hover:text-gray-900">
                        {labels.instagramConfirmBefore} <strong>mauritiusexplored.com</strong> {labels.instagramConfirmAfter}{" "}
                  ...
                  <p className="text-center text-sm text-gray-500">
                    {labels.submittedAgreementBefore}{" "}
                    <Link href="/giveaway/terms" className="text-orange-600 font-medium hover:underline">
                      {labels.termsAndConditions}
                    </Link>
                    {labels.submittedAgreementAfter}
                  </p>
                </form>
              )}
            </div>
