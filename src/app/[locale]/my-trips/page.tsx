...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = getUtilityPageInfo(locale).myTrips;
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 sm:py-12">
        ...
        <div className="flex items-center justify-between mb-8">
          <p className="text-sm text-gray-500">
            {itineraries.length === 0
              ? t.emptySummary
              : t.savedSummary
                .replace("{count}", String(itineraries.length))
                .replace("{label}", itineraries.length === 1 ? t.singularLabel : t.pluralLabel)}
          </p>
          <Link
        ...
        {itineraries.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
            ...
            <h2 className="text-lg font-semibold text-gray-900 mb-2">{t.noTripsTitle}</h2>
            ...
            >
              <ButtonLabel name="createFirstItinerary" />
            </Link>
          </div>
        ) : (
                ...
                <span className="flex flex-col flex-1 min-w-0">
                  <span>
                    ...
                    <span className="text-xs text-gray-500 mt-0.5 block">
                      {itin.trip_start
                        ? new Date(itin.trip_start).toLocaleDateString(t.dateLocale, {
                            day: "numeric",
                            month: "short",
                            year: "numeric",
                          })
                        : `${t.savedPrefix} ${new Date(itin.created_at).toLocaleDateString(t.dateLocale, {
                            day: "numeric",
                            month: "short",
                          })}`}
                    </span>
                  ...
                  <span className="flex items-center gap-1.5 mt-3 flex-wrap">
                    <span className="inline-flex flex-shrink-0 items-center gap-1 px-2.5 py-1 rounded-full bg-gray-50 border border-gray-200 text-xs font-semibold text-orange-600">
                      {itin.stop_count} {itin.stop_count === 1 ? t.stopSingular : t.stopPlural}
                    </span>
                    {itin.is_public && (
                      <span className="inline-flex flex-shrink-0 items-center gap-1 px-2.5 py-1 rounded-full bg-green-50 border border-green-200 text-xs font-semibold text-green-700">
                        {itin.is_anonymous ? t.publicAnonymousLabel : t.publicLabel}
                      </span>
                    )}
                  </span>
                  ...
                  <div className="mt-4 pt-3 border-t border-gray-100 grid grid-cols-2 gap-2">
                    ...
                    <Link
                      href={`/roadtrip-mauritius/create?load=${itin.id}`}
