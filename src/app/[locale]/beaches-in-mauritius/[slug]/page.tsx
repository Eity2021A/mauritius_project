...
export async function generateMetadata({
  ...
  const metaDescription = trimMetaDescription(beach.tagline);
  return {
    title: `${beach.name} ${labels.beach.beachSuffix} - Mauritius | Best Beaches Guide 2026`,
    description: metaDescription,
    alternates: {
      canonical: `/beaches-in-mauritius/${slug}`,
    },
    openGraph: {
      title: `${beach.name} ${labels.beach.beachSuffix} - Mauritius`,
      description: metaDescription,
      images: [{ url: beachImage, width: 1200, height: 630, alt: `${beach.name} ${labels.beach.beachSuffix}` }],
    },
    twitter: {
...
export default async function BeachPage({
  ...
  return (
    <main id="main-content" className="min-h-screen bg-white">
      ...
      <BeachJsonLd
        name={`${beach.name} ${labels.beach.beachSuffix}`}
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: labels.beach.exploreMoreBeaches, url: `${SITE_URL}/beaches-in-mauritius` },
      ...
      <section className="relative h-[40svh] min-h-[220px] md:h-[38vh] md:min-h-[260px] flex items-end">
        <img
          ...
          sizes="100vw"
          alt={formatLabel(labels.beach.beachInMauritiusAlt, { name: beach.name })}
        ...
        <div className="relative z-10 w-full pb-8 md:pb-12 px-4 pt-[calc(env(safe-area-inset-top)+4.5rem)]">
          <div className="container mx-auto max-w-6xl">
            ...
            >
              ...
              <ButtonLabel name="backToBeaches" />
            </Link>
              ...
              <div className="lg:sticky lg:top-24 space-y-6">
                ...
                <div className="bg-white rounded-lg shadow-lg border border-gray-100 p-6">
                  ...
                  <div className="space-y-1">
                    <InfoItem
                      label={labels.common.location}
                      value={beach.info.location}
                      icon={<LocationIcon />}
                      bordered
                    />
                    <InfoItem
                      label={labels.beach.snorkeling}
                      value={beach.categories.includes("snorkeling") ? labels.beach.yes : labels.beach.no}
                      icon={<SnorkelingIcon />}
                      highlight={beach.categories.includes("snorkeling")}
                      bordered
                    />
                    <InfoItem
                      label={labels.beach.beachAmenities}
                      value={beach.categories.includes("amenities") ? labels.beach.available : labels.beach.notAvailable}
                      icon={<AmenitiesIcon />}
                      highlight={beach.categories.includes("amenities")}
                      bordered
                    />
                    <InfoItem
                      label={labels.beach.publicTransport}
                      value={beach.info.publicTransport ? labels.beach.available : labels.beach.notAvailable}
                      icon={<TransportIcon />}
                ...
                {beach.info.hotelsNearby.length > 0 && (
                  <div className="bg-white rounded-lg shadow-lg border border-gray-100 p-6">
                    <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-4">{labels.beach.hotelsNearby}</h3>
                ...
                >
                  {labels.beach.exploreMoreBeaches}
                </Link>
              </div>
            </div>
          </div>
        ...
        <section className="py-10 md:py-12 bg-gray-50">
          <div className="container mx-auto px-4 max-w-6xl">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">
              {formatLabel(labels.beach.moreBeachesIn, { region: beach.region })}
            </h2>
            <p className="text-gray-600 mb-8">{formatLabel(labels.beach.continueCoast, { location: beach.info.location })}</p>
