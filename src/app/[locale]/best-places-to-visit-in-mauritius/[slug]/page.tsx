...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const metaDescription = trimMetaDescription(place.tagline);
  return {
    title: `${place.name} - Mauritius | ${labels.place.bestPlacesToVisit}`,
    ...
    <main id="main-content" className="min-h-screen bg-white">
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: labels.place.placesToVisit, url: `${SITE_URL}/best-places-to-visit-in-mauritius` },
              ...
              <div className="lg:sticky lg:top-24 space-y-6">
                {linkedActivity?.pricing && linkedActivity.pricing.length > 0 && (
                  <div className="space-y-2">
                    ...
                    <p className="text-center text-xs text-gray-500">
                      {labels.common.reservationsPartner}
                    </p>
                  </div>
                )}
                ...
                <div className="bg-white rounded-lg shadow-lg border border-gray-100 p-6">
                  ...
                  <div className="space-y-1">
                    <InfoItem icon={<LocationIcon />} label={labels.common.location} value={place.info.location} bordered />
                    {place.info.openHours && (
                      <InfoItem icon={<DurationIcon />} label={labels.common.openingHours} value={place.info.openHours} bordered />
                    )}
                    {place.info.admission && (
                      <InfoItem icon={<AdmissionIcon />} label={labels.common.admission} value={place.info.admission} bordered />
                    )}
                    {place.info.bestTime && (
                      <InfoItem icon={<CalendarIcon />} label={labels.common.bestTimeToVisit} value={place.info.bestTime} bordered />
                    )}
                  </div>
                </div>
                ...
                >
                  {labels.place.exploreMorePlaces}
                </Link>
              </div>
            </div>
          </div>
        ...
        <section className="py-10 md:py-12 bg-gray-50">
          <div className="container mx-auto px-4 max-w-6xl">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">
              {labels.place.morePlacesToDiscover}
            </h2>
            <p className="text-gray-600 mb-8">{labels.place.continueBest}</p>
                  ...
                  <div className="relative aspect-[16/10] overflow-hidden img-shimmer">
                    {relatedPlace.images.length > 0 ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-gray-200">
                        <span className="text-gray-400">{labels.common.noImage}</span>
                      </div>
                    )}
                  </div>
                  <div className="p-4">
