...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: t.metadata.title, description: t.metadata.description, alternates: { canonical: "/exploring-chamarel" } };
}

export default async function ExploringChamarelPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = getExploringGuide(locale, "chamarel");
  return (
    <main id="main-content" className="min-h-screen bg-white text-[#1c2a2e]">
      ...
      <article className="mx-auto w-full max-w-6xl px-4 pt-24 pb-20 sm:px-6 lg:pt-28 xl:px-0">
        ...
        <section className="mt-20">
          <Image src="/images/quick-trips/chamarel_full_map.png" alt={t.mapAlt} width={1200} height={1200} priority className="h-auto w-full rounded-md" />
        </section>
      </article>
