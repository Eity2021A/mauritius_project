...
export default function CreateItineraryPage() {
  ...
  const ui = getItineraryTranslations(locale).builderUi;
  ...
  const typeFilters = [
    ...
    { key: "beach", label: ui.beaches },
    { key: "activity", label: ui.activities },
    { key: "place", label: ui.places },
  ];
  const backHref = editingId ? "/my-trips" : "/roadtrip-mauritius";
  const backLabel = editingId ? t("backToMyItineraries") : t("backToItineraries");
    ...
    <main id="main-content" className="min-h-screen bg-gray-50">
      ...
      <section className="relative h-[36svh] sm:h-[32svh] print:hidden min-h-[220px]">
        ...
        <div className="absolute inset-0 flex items-center justify-center z-10">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white">
            {editingId ? ui.editTitle : ui.createTitle}
          </h1>
      ...
      <div className="lg:hidden print:hidden sticky top-[calc(env(safe-area-inset-top)+4rem)] z-30 bg-white border-b border-gray-200">
        <div className="flex">
          ...
          >
            {ui.myStops}{stopCount > 0 && ` (${stopCount})`}
          </button>
          <button
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="px-4 py-3 border-b border-gray-100">
                ...
                >
                  ...
                  <span className="font-medium">
                    {draft.tripStart ? `${ui.trip}: ${new Date(draft.tripStart + "T00:00:00").toLocaleDateString(locale === "en" ? "en-GB" : locale, { day: "numeric", month: "short", year: "numeric" })}` : ui.setTripDate}
                  </span>
                ...
                <div id="trip-date-fields" className={draft.tripStart ? "" : "hidden"}>
                  <div className="mt-3">
                    <div>
                      <label className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">{ui.date}</label>
                      <input
                        type="date"
              ...
              {stopCount === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-gray-600 mb-1">{ui.noStops}</p>
                  ...
                  {routeInfo && routeInfo.legs.length > 0 && (
                    <div className="mx-3 mt-2 mb-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{ui.totalDriving}</span>
                        <div className="flex items-center gap-2">
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-gray-50 border border-gray-200 text-xs font-semibold text-orange-600">
                  ...
                  >
                    {saving ? (
                      ...
                      <>
                        ...
                        {editingId ? t("updateItinerary") : t("saveToMyAccount")}
                      </>
                    )}
                  </button>
                  {saveError && (
                ...
                {poolLoading ? (
                  <div className="p-12 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm font-medium text-gray-700">
                      {ui.loadingPool
                        .replace("{beaches}", String(EXPECTED_POOL_COUNTS.beaches))
                        .replace("{activities}", String(EXPECTED_POOL_COUNTS.activities))
                        .replace("{places}", String(EXPECTED_POOL_COUNTS.places))}
                    </p>
                    <p className="text-xs text-gray-400 mt-1">{ui.justMoment}</p>
                  </div>
      ...
      {showMap && (
        <div className="lg:hidden fixed inset-0 z-[80] bg-white flex flex-col print:hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
            <h3 className="text-base font-semibold text-gray-900">{ui.yourRoute}</h3>
            <button
      ...
      <div className="hidden print:block p-8">
        ...
        {routeInfo && (
          <p className="mt-4 text-sm font-medium text-gray-700">
            {ui.totalDriving}: {routeInfo.totalDistanceKm} km · {formatDuration(routeInfo.totalDurationMin)}
          </p>
        )}
      </div>
