...
import { localizePreDesignedItineraries } from "@/data/localized-predesigned-itineraries";

export const legacyMetadata: Metadata = {
  title: "Mauritius Itineraries - Plan Your Trip",
  description: "Plan Mauritius road trips and itineraries with beaches, sights, and scenic drives.",
  openGraph: {
    title: "Mauritius Itineraries - Plan Your Trip",
    description: "Plan Mauritius road trips and itineraries with beaches, sights, and scenic drives.",
    images: [DEFAULT_OG_IMAGE],
  },
  alternates: { canonical: "/roadtrip-mauritius" },
};
...
export default async function RoadTripMauritiusPage({
  params,
}: {
  params?: Promise<{ locale: string }>;
}) {
    ...
    <main id="main-content" className="min-h-screen">
      ...
      <div className="w-full bg-gray-50/80 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
          ...
          {/* Create your itineraries - id for deep-link /roadtrip-mauritius#build-your-own */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
            ...
            {/* AI itinerary */}
            {/* <Link
              href="/roadtrip-mauritius/ai-generate"
            ...
            >
              ...
              <span className="flex flex-col flex-1 min-w-0">
                <span className="font-bold text-gray-900 text-base leading-snug group-hover:text-orange-600 transition-colors flex items-center gap-2 flex-wrap">
                  {t.createCardTitle}
                  <span className="text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded bg-orange-500 text-white">{t.newBadge}</span>
                </span>
                <span className="text-sm text-gray-500 mt-1 block leading-relaxed">
          ...
          {/* Community itineraries - id for nav deep-link /roadtrip-mauritius#community */}
          <section id="community" className="scroll-mt-24">
            <h2 className="text-lg font-semibold text-gray-900 mb-1">{t.communityTitle}</h2>
            <p className="text-sm text-gray-500 mb-4">{t.communitySubtitle}</p>
          ...
          {communityItineraries.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-10 text-center">
              ...
              <p className="text-sm font-medium text-gray-700 mb-1">{t.emptyTitle}</p>
              <p className="text-xs text-gray-400 mb-4">{t.emptyDescription}</p>
              <Link href="/roadtrip-mauritius/create" className="inline-flex items-center gap-2 px-4 py-2 bg-orange-500 text-white text-sm font-medium rounded-lg hover:bg-orange-600 transition-colors">
                {t.createCardTitle}
              </Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {communityItineraries.map((itin) => (
                <div key={itin.id} className="relative">
                  <Link
                    href={`/roadtrip-mauritius/shared/${itin.slug}`}
                  ...
                  >
                    ...
                    <span className="flex flex-col flex-1 min-w-0 justify-between">
                      <span>
                        ...
                        <span className="text-xs text-gray-400 mt-0.5 block">{t.by} {itin.author_name}</span>
                      </span>
                      <span className="flex items-center gap-1.5 mt-3 flex-nowrap overflow-x-auto min-w-0">
                        <span className="inline-flex flex-shrink-0 items-center gap-1 px-2.5 py-1 rounded-full bg-gray-50 border border-gray-200 text-xs font-semibold text-orange-600">
                          {itin.stop_count} {itin.stop_count !== 1 ? t.stopPlural : t.stopSingular}
                        </span>
